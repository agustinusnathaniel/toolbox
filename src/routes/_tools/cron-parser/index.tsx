...
function CronParserPage() {
  ...
  const handleCopyRuns = useCallback(async () => {
    ...
    const text = result.nextRuns.join('\n');
    if (await copy(text, 'runs', 'Copied run times')) {
          ...
          <div className="flex flex-wrap gap-2">
            {CRON_EXAMPLES.map((example) => (
              <Button
                ...
                onPress={() => handleSelectExample(example.expression)}
                size="xs"
              >
                {example.expression}
              </Button>
                  ...
                  {(result.nextRuns ?? []).map((run, index) => (
                    ...
                    >
                      ...
                      <Button
                        ...
                        intent="outline"
                        onPress={() => handleCopySingleRun(run, index)}
                        size="sq-sm"
                      >
                        {copiedKey === `run-${index}` ? (
                          <Check className="size-4 text-success" />
